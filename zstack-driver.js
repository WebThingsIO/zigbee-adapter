...
class ZStackDriver extends ZigbeeDriver {
    ...
    resetZNP() {
        const frame = {
          ...
          this.queueCommandsAtFront([
            ...
            new Command(WAIT_FRAME, {type: cmdType.AREQ, subsys: subSys.SYS, cmd: 0x80, waitTimeout: 5000}) //SYS_RESET_IND
          ]);
    }
        ...
        else
        {
            ...
            if (frame.profileId === PROFILE_ID.ZDO)
            {
                ...
                case zdo.CLUSTER_ID.MANAGEMENT_PERMIT_JOIN_REQUEST:
                    {
                        let dstAddr = parseInt(frame.destination16, 16);
    ...
    onZStackFrame(frame) {
        ...
        if ((frame.type == cmdType.AREQ && frame.subsys == subSys.ZDO && frame.cmd == 0xFF) ||
            frame.drop === true)
        {
            return;
        }
    ...
    parseAREQ(frame) {
        ...
        if (frame.subsys == subSys.ZDO)
        {
            ...
            {
                frame.remote16 = reader.nextString(2, 'hex').swapHex();//reader.nextUInt16LE();
                reader.move(-1);
                ...
                let node = this.adapter.findNodeByAddr16(frame.remote16);
                if (node)
                {
            ...
            else if (frame.cmd == 0xFF) //zdoMsgCbIncomming
            {
                ...
                frame.clusterId = reader.nextString(2, 'hex').swapHex(); //reader.nextUInt16LE().toString(16);
                frame.securityUse = reader.nextUInt8() == 0 ? false : true;
                frame.zdoSeq = reader.nextUInt8();
                frame.destination16 = reader.nextString(2, 'hex').swapHex(); //reader.nextUInt16LE();
                reader.move(-1);
                frame.data = reader.restAll();
        ...
        // else if (frame.subsys == subSys.SAPI)
        // {
        //     console.log(`SAPI AREQ ${frame.cmd}`);
        // }
        ...
        if (frame.subsys == subSys.SYS)
        {
            ...
            else if (frame.cmd == 0x08) // OSAL_NV_READ
            {
                if (frame.payload[0] == 0x00) //success
                {
                    ...
                    {
                        let panID = frame.payload.readUInt16LE(2);
                        if (panID == 0xFFFF)
                        {
